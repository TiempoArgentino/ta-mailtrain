...
class Mailtrain_API_Curl
{
    ...
    public function desuscribe($lists,$email)
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
        CURLOPT_URL => get_option('mailtrain_api_option_name')['url_mailtrain'] . '/api/unsubscribe/'.$lists. '?access_token=' . get_option('mailtrain_api_option_name')['access_token_0'],
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
